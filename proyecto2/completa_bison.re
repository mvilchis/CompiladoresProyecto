...
file_input_aux: fi_aux | %empty 
fi_aux: fi_aux file_input_aux | NEWLINE | stmt
eval_input: testlist eval_input_aux FIN
eval_input_aux: ei_aux | %empty
ei_aux: ei_aux eval_input_aux | NEWLINE 

decorator: AT dotted_name [ APAREN [arglist] CPAREN ] NEWLINE
decorators: decorators decorator | decorator
decorated: decorators decorated_aux
decorated_aux: classdef | funcdef
funcdef: DEF IDENTIFICADOR parameters DPUNTO suite

parameters: APAREN [varargslist] CPAREN
varargslist: varargslist_a | varargslist_b
varargslist_a: varargslist_a_aux_a varargslist_a_aux_b
varargslist_a_aux_a: varargslist_a_aux_a fpdef [IGUAL test] COMA | %empty
varargslist_a_aux_b: ASTERISCO IDENTIFICADOR [COMA DASTERISCO IDENTIFICADOR] | DASTERISCO IDENTIFICADOR
varargslist_b: fpdef [IGUAL test] varargslist_b_aux [COMA]
varargslist_b_aux: varargslist_b_aux COMA fpdef [IGUAL test] | %empty

fpdef: IDENTIFICADOR | APAREN fplist CPAREN
fplist: fpdef fplist_aux [COMA]
fplist_aux:  fp_aux | %empty
fp_aux: fp_aux fplist_aux | COMA fpdef


stmt: simple_stmt | compound_stmt
simple_stmt: small_stmt simple_stmt_aux [PCOMA] NEWLINE
simple_stmt_aux: simple_stmt_aux PCOMA small_stmt | empty
small_stmt: expr_stmt | print_stmt  | del_stmt | pass_stmt | flow_stmt |
             import_stmt | global_stmt | exec_stmt | assert_stmt
expr_stmt: testlist augassign expr_stmt | expr_stmt_aux_b
expr_stmt_aux_a: yield_expr | testlist
expr_stmt_aux_b:  expr_stmt_aux_b IGUAL es_aux_b | %empty; 
es_aux_b: yield_expr | testlist


augassign: MASIGUAL | MENOSIGUAL | ASTIGUAL | DIAIGUAL | PORIGUAL | AMPIGUAL | PIPEIGUAL | CIRCIGUAL |
            DMENORIGUAL | DMAYORIGUAL | DASTIGUAL | DDIAIGUAL

print_stmt: PRINT  [ test print_stmt_aux_a [COMA] ] |
                      DMAYOR test [ print_stmt_aux_b [COMA] ]
print_stmt_aux_a: print_stmt_aux_a COMA test | %empty
print_stmt_aux_b: print_stmt_aux_b COMA test | COMA test
