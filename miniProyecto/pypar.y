...
file_input: file_input NEWLINE {$$=$1;}| stmt file_input {$$=ast->bHERMANOSNode($1, $2);}| {} 
;
...
simple_stmt: small_stmt simple_stmt_aux ss_aux NEWLINE {$$=ast->bHERMANOSNode($1,$2);}
;
ss_aux: PCOMA{}
| {} 
;
...
;
test: or_test s15 {$$=ast->bHERMANOSNode($1,$2);}
;
/*COMPARADORES*/
or_test: and_test or_test_aux  {$$=ast->bORNode($1,$2);}

;
or_test_aux:or_test_aux  OR and_test {$$=ast->bORNode($1,$3);} |{}   
;
...
;
comparison_aux:  comparison_aux comp_op expr {$$=ast->bHERMANOSNode($1,$2);} |{$$=ast->bHERMANOSNode();}  
;
...
;
expr_aux:expr_aux   PIPE xor_expr  {$$=ast->bPIPENode($1,$3);}|{}   
;
...
;
exprlist_aux: exprlist_aux COMA expr {$$=$3;} |{}   
;
testlist: test testlist_aux comaS {$$=$1;}
;
testlist_aux:  testlist_aux COMA test {$$=$3;}|{}   
;
